            ...
            <tbody>
                ...
                @php
                    ...
                    $coincide = !empty($busqueda) && (
                        $registro->idpay == $busqueda || // Buscar en ID (comparación directa)
                ...
                @endphp
                    ...
                    <tr style="{{ $coincide ? 'background-color: yellow;' : '' }}">
                        <td>{{ $registro->idpay }}</td>
                        <td>{{ $registro->user->nombres }}</td>
                        <td>{{ $registro->user->apellidos }}</td>
                        <td>{{ $registro->user->email ?? 'No disponible' }}</td>
                        <td>{{ $registro->diplomado->titulo ?? 'No disponible' }}</td>
                        <td>{{ $registro->diplomado->modalidad ?? 'No disponible' }}</td>
                        <td>{{ $registro->situacion ?? '' }}</td>
                        <td>{{ $registro->monto }}</td>
                        <td>{{ \Carbon\Carbon::parse($registro->fecha)->format('d/m/Y') }}</td>
                        <td>{{ $registro->tipo_pago ?? 'No disponible' }}</td>
                        <td>{{ $registro->nmr_trans ?? 'No disponible' }}</td>
                        <td>
                            <button 
                                type="button" 
                                class="btn btn-outline-info btn-sm" 
                                onclick="window.location.href='https://www.mayugo.net/degree/mifactura.php?email={{ urlencode($registro->user->email ?? '') }}'">
